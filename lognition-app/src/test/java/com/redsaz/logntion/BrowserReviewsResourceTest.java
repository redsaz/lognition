...
public class BrowserReviewsResourceTest {
  ...
  public void testUploadAttachments() {
    ...
    Attachment expectedSource =
        new Attachment(
            ...
            "It is an image",
            "image/jpeg",
            actualSource.getUploadedUtcMillis());

    assertEquals(
        expectedSource, actualSource, "The attachment details to be stored are incorrect.");
  }
  ...
  public void testUploadAttachments_noOptionalParams() throws IOException {
    ...
    when(reviews.getAttachmentData(anyLong(), any(String.class)))
        .thenReturn(new BufferedInputStream(new FileInputStream("src/test/resources/image.jpg")));
    ...
    assertEquals(
        expectedSource,
        actualSource,
        "The creation attachment details to be stored are incorrect.");
