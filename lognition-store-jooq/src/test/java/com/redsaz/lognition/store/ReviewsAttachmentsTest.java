...
public class ReviewsAttachmentsTest {
  ...
  @Rule public TemporaryFolder attachTempDir = new TemporaryFolder();
  ...
  public void testCreateAndDownloadAttachment() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      Attachment source =
          new Attachment(
              0L, "", path, "Image", "It is an image", "image/jpeg", System.currentTimeMillis());
      Attachment actual = reviewsSvc.putAttachment(review.getId(), source, data);
      DigestInputStream actualData =
          wrapInDigestStream(reviewsSvc.getAttachmentData(review.getId(), source.getPath()));
      ...
      assertFalse(
          "Must not store the file with the actual filename of the attachment.",
  ...
  public void testCreateAttachment_existingName() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      assertEquals("Owner", "reviews/1", actual.getOwner());
      assertEquals(
          "One file should exist after adding another attachment with same path",
          Files.list(attachDir.toPath()).count(),
          1L);
    }
  }
  ...
  public void testCreateAttachment_samePathDifferentReviews() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      DigestInputStream actualData1 =
          wrapInDigestStream(reviewsSvc.getAttachmentData(review1.getId(), source1.getPath()));
      ...
      DigestInputStream actualData2 =
          wrapInDigestStream(reviewsSvc.getAttachmentData(review2.getId(), source2.getPath()));
  ...
  public void testUpdateAttachment_cannotUpdateUnchangeableFields()
    ...
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      Attachment changed =
          new Attachment(1234L, original.getOwner(), original.getPath(), null, null, null, 4321L);
      ...
      assertEquals(
          "UploadedUtcMillis", original.getUploadedUtcMillis(), result.getUploadedUtcMillis());
  ...
  public void testMoveAttachment() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      DigestInputStream actualData =
          wrapInDigestStream(reviewsSvc.getAttachmentData(review.getId(), actual.getPath()));
      assertEqualData(data, actualData);

      // and the attachment is not retrievable from the original path.
      assertNull(
          "Attachment should not be retrievable from original path.",
          reviewsSvc.getAttachment(review.getId(), source.getPath()));
    }
  }
  ...
  public void testMoveAttachment_overwritesOtherAttachment() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      // and the data is downloadable from the new path,
      DigestInputStream actualData =
          wrapInDigestStream(reviewsSvc.getAttachmentData(review.getId(), actual1.getPath()));
      assertEqualData(data1, actualData);

      // and the attachment is not retrievable from the original path.
      assertNull(
          "Attachment should not be retrievable from original path.",
          reviewsSvc.getAttachment(review.getId(), source1.getPath()));
  ...
  public void testDeleteAttachment() throws IOException, SQLException {
    try (CloseableConnectionPool cp = createConnectionPool();
      ...
      try {
        ...
        fail(
            "Exception should be thrown when attempting to fetch data for attachment that was deleted.");
      ...
      // (impelemntation details) and the old attachment is deleted.
      assertEquals(
          "The attachment file should be deleted when the attachment was deleted.",
          Files.list(attachDir.toPath()).count(),
          0L);
    }
  }
  ...
  private static void assertEqualData(DigestInputStream expected, DigestInputStream actual)
      throws IOException {
    ...
    assertArrayEquals(
        "Downloaded attachment data doesn't match what was uploaded",
