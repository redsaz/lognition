...
public class JooqLogsService implements LogsService {
  ...
  public Log create(Log source) {
    ...
    try (Connection c = dataSource.getConnection()) {
      ...
      LogRecord result =
          context
              .insertInto(LOG, LOG.STATUS, LOG.URI_NAME, LOG.NAME, LOG.DATA_FILE, LOG.NOTES)
              .values(
  ...
  public void delete(long id) {
    ...
    try (Connection c = dataSource.getConnection()) {
      ...
      throw new AppServerException(
          "Failed to delete log_id=" + id + " because: " + ex.getMessage(), ex);
    }
  }
  ...
  public List<Label> setLabels(long logId, Collection<Label> labels) {
    ...
    try (Connection c = dataSource.getConnection()) {
      ...
      Map<String, Label> plannedLabels =
          labels.stream().collect(Collectors.toMap(Label::getKey, Function.identity()));
      Map<String, String> existingLabels =
          context.selectFrom(LABEL).where(LABEL.LOG_ID.eq(logId)).stream()
              .collect(Collectors.toMap(LabelRecord::getKey, LabelRecord::getValue));

      plannedLabels.forEach(
          (k, v) -> {
      ...
      Set<String> toDeletes =
          existingLabels.keySet().stream()
      ...
      if (!toAdds.isEmpty()) {
        ...
        InsertValuesStep3<LabelRecord, Long, String, String> insert =
            context.insertInto(LABEL).columns(LABEL.LOG_ID, LABEL.KEY, LABEL.VALUE);
  ...
  public List<Label> getLabels(long logId) {
    try (Connection c = dataSource.getConnection()) {
      ...
      List<Label> labels =
          context.selectFrom(LABEL).where(LABEL.LOG_ID.eq(logId)).fetchInto(r2lHandler).getValues();
  ...
  private static class LabelSelectorToSelect implements LabelSelectorExpressionListener {
    ...
    public void exists(String labelName) {
      ...
      Condition inCondition =
          LOG.ID.in(context.selectDistinct(LABEL.LOG_ID).from(LABEL).where(condition));
      conditions.add(inCondition);
    }
