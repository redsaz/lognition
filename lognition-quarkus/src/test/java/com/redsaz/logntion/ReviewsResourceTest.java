...
public class ReviewsResourceTest {
    ...
    public void testListReviews() {
        when(reviews.list()).thenReturn(Arrays.asList(
                new Review(1L, "test1", "Test1", "description", 1231231234000L, 1231231234000L, "test"),
                new Review(2L, "test2", "Test2", "description", 1231231234000L, 1231231234000L, "test")
        ));
        given()
                .when().accept(LognitionMediaType.REVIEW_V1_JSON).get("/reviews")
                .then()
                .statusCode(200)
    ...
    public void testGetReview() {
        when(reviews.get(eq(1L))).thenReturn(
                new Review(1L, "test1", "Test1", "description", 1231231234000L, 1231231234000L, "test")
        );
        given()
                .when().accept(LognitionMediaType.REVIEW_V1_JSON).get("/reviews/1")
                .then()
                .statusCode(200)
                .body(containsString("test1"));
    }
    ...
    public void testCreateReview() {
        when(reviews.create(any())).thenReturn(
                new Review(1L, "test1", "Test1", "description", 1231231234000L, 1231231234000L, "test")
        );
        given()
                .when().accept(LognitionMediaType.REVIEW_V1_JSON)
