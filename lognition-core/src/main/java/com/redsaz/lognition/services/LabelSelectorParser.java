...
public class LabelSelectorParser {
  ...
  private static final Pattern LABEL_SELECTOR_PATTERN =
      Pattern.compile("(!=|[,()=!]|in|notin)|([-._a-zA-Z0-9]+)|(\\S)", Pattern.CASE_INSENSITIVE);
  ...
  private static LabelSelectorExpression parsize(List<Token> tokens) {
    ...
    */
    ...
    // !key3
    // In SQL:
    // SELECT DISTINCT log_id FROM label l1 LEFT JOIN label l2 ON l1.log_id = l2.log_id AND l2.key =
    // 'key1' WHERE l2.key IS NULL;
    // Or:
    // SELECT DISTINCT log_id FROM label WHERE NOT EXISTS (SELECT log_id FROM label WHERE key =
    // 'key3')
    //
   ...
   *
   * <p>That is, given this text <code>key1 = value1</code>, split into these strings: "key1", "=",
   ...
   * because that fits what can be a KEY token. But, that same pattern is also a valid VALUE token.
   * We know that it isn't a VALUE token because we're at the START state, where only a KEY token or
   * a NOT_EXISTS token are possible.
   *
   * <p>To advance from START state, we can go to LABEL_KEY state. Acceptable pretokens can now be
   * "=", "!=", "in", "notin", ",", or absolutely nothing (if nothing, the key is part of an
   * "exists" expression).
   *
   * <p>So, to use States, start with the START state, feed in a pretoken to get the token and
   * state, then parse the next token based on the new state, and repeat until all pretokens are
   * converted:
   *
   * <pre>
   * State state = States.START;
  ...
  private static class NameState implements State {
    ...
    public State expressionize(ComposibleLabelSelectorExpression expression, Token token) {
      ...
      if (token == Tokens.EQUALS || token == Tokens.NOT_EQUALS) {
        ...
        throw new LabelSelectorSyntaxException(
            "Label Selector syntax error. Expected one of: [\""
                + Tokens.AND
                + "\", \""
                + Tokens.EQUALS
                + "\", \""
  ...
  private static class ScalarOpState implements State {
    ...
    public State expressionize(ComposibleLabelSelectorExpression expression, Token token) {
      ...
      if (token instanceof KeyOrValueToken) {
        ...
        throw new LabelSelectorSyntaxException(
            "Label Selector syntax error. Expected "
                + "label value but got "
                + token
                + " instead.");
      }
  ...
  private static enum States implements State {
    ...
    FINISH {
      ...
      public State expressionize(ComposibleLabelSelectorExpression expression, Token token) {
        ...
        throw new LabelSelectorSyntaxException(
            "Label Selector is finished, yet " + "encountered: " + token);
      }
    },
    NOT_EXISTS {
      @Override
      public State expressionize(ComposibleLabelSelectorExpression expression, Token token) {
        ...
        throw new LabelSelectorSyntaxException(
            "Label Selector syntax error. Expected " + "label key but got " + token + " instead.");
      }
    },
    LIST_START {
      @Override
      public State expressionize(ComposibleLabelSelectorExpression expression, Token token) {
        if (token instanceof KeyOrValueToken) {
          ...
          throw new LabelSelectorSyntaxException(
              "Label Selector syntax error. Cannot "
